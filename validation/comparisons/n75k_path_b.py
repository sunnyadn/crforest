...
and to add a peak-RSS column comparable across libraries.

Each (lib, seed, cores) cell runs in its own subprocess wrapped by
`/usr/bin/time -v` so peak RSS is measured uniformly. Reads the staged
CHF cohort at /tmp/chf_2012_*.parquet (run
validation/spikes/kappa/exp1_chf_smoke.py once to stage it).

Run on the bench host (Linux):
  PYTHONUNBUFFERED=1 ~/.local/bin/uv run --extra dev \\
    python -u validation/comparisons/n75k_path_b.py --seeds 42,43,44

Output: /tmp/n75k_path_b.parquet + .fingerprint.json.
"""
...
HERE = THIS.parent
RFSRC_R = HERE / "_n75k_path_b_rfsrc.R"

CHF_PATH = Path("/tmp/chf_2012_clean.parquet")
...
def main() -> None:
    ...
    parser.add_argument(
        "--seeds", default="42,43,44", help="comma-sep seeds; default 3 (OMP-off is the long pole)"
    )
    parser.add_argument(
        "--cores-on",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=14400,
        help="per-cell timeout in s (default 4hr; OMP-off can be slow)",
    )
    ...
    if not CHF_PATH.exists():
        sys.exit(f"missing {CHF_PATH}; run validation/spikes/kappa/exp1_chf_smoke.py first")
