"""Paired rfSRC OMP-on vs comprisk matched-pair on SEER breast cancer.
...
Run on a Linux box with rfSRC + comprisk installed:
...
def child_comprisk(seed: int, n_jobs: int) -> None:
    """Single comprisk fit on staged SEER cohort."""
    import numpy as np
    import pandas as pd
...
def main() -> None:
    ...
    parser.add_argument(
        "--cells",
        default="rfsrc_on,comprisk",
        help="comma-sep subset of {rfsrc_on, comprisk}",
    )
    ...
    if len(ok):
        ok["cell"] = ok.apply(
            lambda r: "rfsrc_on" if r["lib"] == "rfsrc" else "comprisk",
            axis=1,
        )
        agg = (
