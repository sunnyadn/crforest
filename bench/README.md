...
Reproducible head-to-head benchmarks vs `randomForestSRC`. Wall-time + parallel-efficiency rows append to `results/results.csv` so you can stack runs across hardware, parameter regimes, and comprisk commits.
...
```bash
# comprisk at its default leaf size, all cores, ntree=100
python -m bench.run_comprisk --ntree 100 --leaf 3 --jobs -1 --label "$(hostname)"
...
| `leaf` (comprisk) / `nodesize` (rfSRC) | 3 / 15 | Each library's own default; vary to study sensitivity |
| `nsplit` | 10 | Random-split candidates per feature; matches rfSRC v3+ default |
| `n_bins` | 256 | comprisk histogram resolution (rfSRC has no analog) |
...
- **comprisk parallelizes both across trees (joblib) and within trees (numba).** The bench harness only logs main-process CPU time; for full-machine CPU usage, use `validation/profile_fit.py` with cProfile.
...
- Forcing rfSRC to `nodesize=3` (comprisk's default leaf) at large n triggers OOM in the rfSRC ensemble at modest ntree. We saw this at n=60k, ntree=500, 128 GB allocated: rfSRC `OUT_OF_MEMORY`. The aligned-deep-trees regime is **comprisk-friendly** by design and should not be the headline number.
- Forcing comprisk to skip its histogram and use exact split scans is not exposed via the API; that algorithmic difference is permanent and not a knob.
...
3. **comprisk-defaults aligned**: both at `nodesize=leaf=3`. Stresses rfSRC outside its design point; useful for showing scaling failure modes (OOM + super-linear walls).
...
# comprisk, both leaf settings, both ntree=100/300/500
for leaf in 3 15; do
  for ntree in 100 300 500; do
    python -m bench.run_comprisk --ntree "$ntree" --leaf "$leaf" --jobs -1 \
      --label "$(hostname)"
  done
done
...
| `leaf_or_nodesize` | comprisk `min_samples_leaf` / rfSRC `nodesize` |
| `nsplit` | Random-split candidates per feature |
| `n_bins` | comprisk only; blank for rfSRC |
...
| `commit` | comprisk git commit hash (short); blank for rfSRC |
| `notes` | Free-form |
